...
class Datatables {
    ...
    public function join($table, $cond, $type = NULL) {
        ...
        $this->ci->db->join($table, $cond, $type);
        return $this;
    }
...
//        pr($val);exit;
//        $this->ci->db->like('vFirstName',$val);
//      $this->where[] = array($key_condition, $val, $backtick_protect);
//        pr($key_condition);exit;
        $like_data = '';
        for ($i = 0; $i < count($key_condition); $i++) {
            if ($i == 0) {
                //$this->ci->db->like($key_condition[$i], $val);
                $like_data = "($key_condition[$i] like '%$val%'";
            } else {
                $key_condition[$i] = (strpos($key_condition[$i], ' as ') > 0 ? substr($key_condition[$i], 0, strpos($key_condition[$i], ' as ')) : $key_condition[$i]);
                //$this->ci->db->or_like($key_condition[$i], $val);
                $like_data .= " or $key_condition[$i] like '%$val%'" . (($i == count($key_condition) - 1) ? ')' : '');
            }
        }
        //$this->filter[] = array($like_data, NULL, $backtick_protect);
     ...
     * @return string
     */
    public function generate($tbl, $charset = 'UTF-8') {
        $this->get_paging();
        $this->get_ordering();
        $this->get_filtering();
        return $this->produce_output($tbl, $charset);
    }
...
//              pr($this->ci->input->get());exit;
    ...
    protected function get_ordering() {
        ...
        elseif ($this->ci->input->post('columns'))
        ...
        foreach ($mColArray as $key => $value) {
            if ($value['sortable'] == 'true' && in_array($mColArray[$key]['name'], $columns) && $search['value'] != '') {
                $this->ci->db->order_by($mColArray[$key]['name'], '');
            }
        }
//        pr($columns);exit;
//        for ($i = 0; $i < intval(count($columns)); $i++){
//            if ($mColArray[$i]['sortable'] == 'true'){
//                echo 'hdf';exit;
//                $this->ci->db->order_by($mColArray[$i], $order[0]['dir']);
//            }
//        }
    }

    function get_sorting() {
        $col = $this->ci->input->post('order');
        $coldata = $this->ci->input->post('columns');
        $colid = $col[0]['column'];
        $coldir = $col[0]['dir'];
//        pr($coldata[$colid]['orderable']);exit;
        if ($coldata[$colid]['orderable'] == 'true')
            $this->ci->db->order_by($coldata[$colid]['name'], $coldir);
    }
    ...
    function get_filtering() {
        ...
        if ($search != '') {
            foreach ($mColArray as $key => $value) {
                if ($value['searchable'] == 'true' && in_array($mColArray[$key]['name'], $columns) && $search['value'] != '') {
                    $sWhere .= $this->select[$mColArray[$key]['name']] . " LIKE '%" . $search['value'] . "%' OR ";
                }
            }
        }
        ...
        //$this->filter[] = array('(' . $sWhere . ')', NULL, false);
//        for ($i = 0; $i < intval($this->ci->input->post('iColumns')); $i++) {
//            if (isset($_POST['search_' . $i]) && $this->ci->input->post('search_' . $i) != '' && in_array($mColArray[$i], $columns)) {
//                $miSearch = explode(',', $this->ci->input->post('search_' . $i));
//
...
//        }
     ...
     * @return mixed
     */
    function get_display_result($tbl) {
        $data = $this->ci->db->get();
        //echo $this->ci->db->last_query();exit;
        $count = count(explode(" ",trim($tbl))); 
        if ($count > 1) {
            $arr = explode("as", $tbl);
            $tbl = trim($arr[count($arr) - 1]);
        }

        $companyId = $this->ci->config->item('CRM_USER_COMPANY_ID');
        $userId = $this->ci->session->userdata('iUserId');

        $folder_path = $this->ci->config->item('upload_path') . 'queries/' . $companyId . '/' . $userId . '/';
        $this->ci->general->createfolder($folder_path);

        $f = fopen($folder_path . $tbl . '.text', 'w+');
        fwrite($f, $this->ci->db->last_query());
        fclose($f);
        return $data;
    }
     ...
     * @return string
     */
    protected function produce_output($tbl, $charset) {
        $aaData = array();
        $rResult = $this->get_display_result($tbl);
        ...
        $columns = array_diff($this->columns, $this->unset_columns);
        $columns = array_merge_recursive($columns, array_keys($this->add_columns));
