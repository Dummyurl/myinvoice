...
Class Model_support extends CI_Model {
    ...
    public function authenticate($username, $password) {
        ...
        if (is_array($record) && count($record) > 0) {
            ...
            $this->session->set_userdata("ADMIN_UNAME", $record[0]["Username"]);
            $this->session->set_userdata("ADMIN_FNAME", $record[0]["Firstname"]);
            $this->session->set_userdata("ADMIN_LNAME", $record[0]["Lastname"]);
            $this->session->set_userdata("ADMIN_EMAIL", $record[0]["Email"]);
            $this->session->set_userdata("ADMIN_IMAGE", $record[0]["ProfileImage"]);
            $this->errorCode = 1;
        } else {
            $this->errorCode = 0;
    ...
    function getGstDetail() {
        $this->db->select('GSTPercentage');
        $this->db->where('ID', 1);
        $array = $this->db->get('tbl_setting')->result_array();
        return $array;
    }
